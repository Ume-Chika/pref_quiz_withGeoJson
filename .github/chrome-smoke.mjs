...
try {
  ...
  assert(resultSummary.score > 0 && resultSummary.correct === "10/10" && resultSummary.rate === "100%" && resultSummary.combo === "10" && resultSummary.timeouts === "0" && resultSummary.review.includes("現在の苦手：") && resultSummary.review.includes("次の復習候補：") && resultSummary.record && resultSummary.mistakesHidden && resultSummary.learning.includes('基本を習得'), "10問結果画面の理解度、集計、苦手または次回復習表示が不正です");
  ...
  assert(await evaluate(cdp, `(() => { const svg=document.querySelector('#study-map-canvas svg'); return [...svg.querySelectorAll('.map-prefecture[data-code]')].every(path=>{const box=path.getBBox(); return [box.x,box.y,box.width,box.height].every(Number.isFinite)&&box.width>0&&box.height>0&&box.x+box.width>0&&box.y+box.height>0&&box.x<700&&box.y<540;}); })()`), "47都道府県のいずれかが白地図の表示域外または面積0です");
  ...
  for (let question = 1; question <= 30; question++) {
    await waitFor(cdp, `document.querySelector('#game-screen:not([hidden])') && document.querySelectorAll('input[name="answer"]').length===4 && !document.querySelector('#feedback-dialog').open`);
  ...
  assert(await evaluate(cdp, `document.querySelector('#result-score').textContent==='1000' && document.querySelector('#result-correct').textContent==='30/30' && document.querySelector('#result-rate').textContent==='100%' && document.querySelector('#result-score-label').textContent.includes('偶然正解') && document.querySelector('#result-review').textContent.includes('理解度指数へ影響しません') && document.querySelector('#replay-button').textContent.includes('おすすめ学習')`), "実力テストの結果または学習への導線が不正です");
  ...
  for (const [type, skill, code, correct] of modes) {
    ...
    if (type === "reveal") {
      assert(await evaluate(cdp, `document.querySelector('#timer-text').textContent==='観察中' && [...document.querySelectorAll('input[name="answer"]')].every(input=>input.disabled)`), "じわじわ表示の観察時間が回答時間を消費しています");
      await waitFor(cdp, `document.querySelector('#timer-text').textContent!=='観察中'`, 7_000);
    }
