...
try {
  ...
  for (const [type, skill, code, correct] of modes) {
    ...
    assert(await evaluate(cdp, `[...document.querySelectorAll('.answer-option label')].filter(label => label.getClientRects().length).every(label => label.getBoundingClientRect().height >= 44) && getComputedStyle(document.querySelector('#keyboard-hint')).display !== 'none'`), `${type}: タップ領域またはキー案内が不正です`);
    ...
    if (["shapeMemory", "flash", "mapFlash", "mapMemory", "shapeLocate"].includes(type)) {
      ...
      await waitFor(cdp, `document.querySelector('#timer-text').textContent !== '記憶中'`, type === "shapeMemory" ? 5_000 : 4_000);
