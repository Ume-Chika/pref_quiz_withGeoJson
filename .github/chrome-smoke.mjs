...
try {
  ...
  await evaluate(cdp, `document.querySelector('#study-map-button').click(); true`);
  await waitFor(cdp, `document.querySelector('#study-map-dialog').open`);
  assert(await evaluate(cdp, `document.querySelector('#study-map-zoom-value').textContent==='100%' && document.querySelector('#study-map-zoom-out').disabled && document.querySelector('#study-map-zoom-reset').disabled`), "白地図の初期倍率またはボタン状態が不正です");
  ...
  for (const [type, skill, code, correct] of modes) {
    ...
    if (type === "regionShape") assert(await evaluate(cdp, `(() => { const stage=document.querySelector('#visual-stage'); const exposed=stage.textContent+[...stage.querySelectorAll('[aria-label]')].map(item=>item.getAttribute('aria-label')).join(''); return stage.querySelectorAll('.map-prefecture').length===6 && stage.querySelectorAll('.map-prefecture.target').length===1 && stage.querySelector('.map-prefecture.target[data-map-code="02"]') && !exposed.includes('東北地方'); })()`), "地方の形、対象県の区別または答えの非露出が不正です");
