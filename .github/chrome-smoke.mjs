...
try {
  ...
  for (const [type, skill, code, correct] of modes) {
    ...
    assert(await evaluate(cdp, `document.documentElement.scrollWidth <= innerWidth && [...document.querySelectorAll('.answer-option label')].filter(label => label.getClientRects().length).every(label => label.getBoundingClientRect().height >= 44) && getComputedStyle(document.querySelector('#keyboard-hint')).display !== 'none'`), `${type}: スマートフォン表示、タップ領域、キー案内のいずれかが不正です`);
